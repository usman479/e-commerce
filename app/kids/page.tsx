...
async function getProducts({ start, end }: { start: number, end: number }) {
  const data = await client.fetch(`*[_type=="product" && category->name == 'Kids'] [${start}...${end}] | order(publishedAt desc){
    price, 
    _id,
    title,
    image,
  ...
  const totalRecords = await client.fetch(`count(*[_type == "product && category->name == 'Kids"])`);
  ...
  return (
    <WrapperMaxWidth>
      ...
      <div className='flex gap-x-4 justify-center items-end my-8'>
        {Number(currentPage) > 1 ? <Link href={`/kids?page=${+currentPage-1}`} className='select-none cursor-pointer bg-gray-200 p-2 text-center w-10 h-10 flex justify-center items-center hover:-translate-x-1 duration-300'>{'<<'}</Link> : ''}
        {pages.map(page => (
          // <p className={page == Number(currentPage) ? 'text-white font-bold text-lg' : '' +  'bg-gray-400 p-2 w-8 text-center'} >{page}</p>
          <Link key={page} href={`/kids?page=${page}`} className={cn('select-none cursor-pointer bg-gray-200 p-2  w-10 h-10 flex items-center justify-center', {
            'text-orange-400': page == currentPage,
            'font-bold': page == currentPage,
            // 'text-lg': page == currentPage
          })} >{page}</Link>
        ))}
        {Number(currentPage) < maxPages ? <Link href={`/kids?page=${+currentPage+1}`} className='select-none cursor-pointer bg-gray-200 p-2 text-center w-10 h-10 flex justify-center items-center hover:translate-x-1 duration-300'>{'>>'}</Link> : ''}
      </div>
    </WrapperMaxWidth>
  )
}
